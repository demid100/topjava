...
public class UserMealsUtil {
    public static void main(String[] args) {
        ...
        List<UserMealWithExceed> filteredWithExceeded = getFilteredWithExceeded(mealList, LocalTime.of(7, 0), LocalTime.of(12, 00), 2000);
        filteredWithExceeded.forEach(System.out::println);
    }

    public static List<UserMealWithExceed> getFilteredWithExceeded(List<UserMeal> mealList, LocalTime startTime, LocalTime endTime, int caloriesPerDay) {

        Map<LocalDate, Integer> sumCalloriesByDay = mealList
                .stream()
        ...
        return mealList
                .stream()
                .map(userMeal -> {
                    int callDay = sumCalloriesByDay.get(userMeal.getDateTime().toLocalDate());
                    boolean exceed = false;
                    if (callDay > caloriesPerDay) exceed = true;
